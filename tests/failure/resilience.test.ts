...
describe('System Resilience & Failure Handling', () => {
  test('should ignore duplicated webhooks', async () => {
    const { user, account } = await createUserWithWallet(1);
  ...
  test('should handle out-of-order events safely', async () => {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // Use account.id from closure instead of fetching again to avoid potential null issues in tx context
      const targetWalletId = account.id;
      
      const credited = await tx.accountTransaction.findFirst({
        where: { accountId: targetWalletId, type: 'DEPOSIT', amount }
      });
      if (!credited) {
        await tx.balance.updateMany({
          where: { accountId: targetWalletId, currency: 'EUR' },
          data: { amount: { increment: amount } }
        });
    ...
    const transactions = await prisma.accountTransaction.findMany({
      where: { accountId: account!.id, type: 'DEPOSIT', amount }
    });
  ...
  test('should not credit balance on Stripe failure', async () => {
    const { user, account } = await createUserWithWallet(1);
    const startBalanceRecord = await prisma.balance.findUnique({ where: { accountId_currency: { accountId: account.id, currency: 'EUR' } } });
    const amount = 75;
    ...
    const endBalanceRecord = await prisma.balance.findUnique({ where: { accountId_currency: { accountId: account.id, currency: 'EUR' } } });
  ...
  test('should reject transfer with insufficient balance', async () => {
    const { user: sender, account: senderWallet } = await createUserWithWallet(1);
    const { user: receiver, account: receiverWallet } = await createUserWithWallet(2);

    await prisma.balance.updateMany({
      where: { accountId: senderWallet.id, currency: 'EUR' },
      data: { amount: 50 }
    });
    ...
    await expect(
      prisma.$transaction(async (tx) => {
        const senderBalance = await tx.balance.findUnique({ where: { accountId_currency: { accountId: senderWallet.id, currency: 'EUR' } } });
        ...
        await tx.balance.updateMany({
          where: { accountId: receiverWallet.id, currency: 'EUR' },
  ...
  test('should reject operation above KYC limit', async () => {
    const { user, account } = await createUserWithWallet(0);
