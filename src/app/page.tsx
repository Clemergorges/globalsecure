  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <header className="sticky top-0 z-50 w-full border-b bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/60">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-xl tracking-tight text-cyan-900">GlobalSecure</span>
          </div>
          <nav className="hidden md:flex gap-6 text-sm font-medium text-gray-600">
            ...
            <Link href="#pricing" className="hover:text-cyan-600 transition-colors">Taxas</Link>
          </nav>
          <div className="flex items-center gap-3">
            <Link href="/auth/login">
              <Button variant="ghost" className="font-semibold text-gray-700 hover:text-cyan-600 hover:bg-cyan-50">
                Entrar
              </Button>
            </Link>
            <Link href="/auth/register">
              <Button className="bg-cyan-600 hover:bg-cyan-700 text-white font-semibold shadow-sm">
                Abrir Conta Grátis
              </Button>
            </Link>
          </div>
      ...
      <main className="flex-1">
        {/* Hero Section */}
        <section className="relative overflow-hidden pt-16 pb-24 md:pt-24 md:pb-32 lg:pt-32 bg-gradient-to-b from-cyan-50 to-white">
           {/* Background decorative elements */}
           <div className="absolute inset-0 z-0 opacity-30">
              <div className="absolute top-0 left-1/4 w-96 h-96 bg-cyan-200 rounded-full blur-3xl mix-blend-multiply filter animate-blob"></div>
              <div className="absolute top-0 right-1/4 w-96 h-96 bg-blue-200 rounded-full blur-3xl mix-blend-multiply filter animate-blob animation-delay-2000"></div>
              <div className="absolute -bottom-32 left-1/3 w-96 h-96 bg-purple-200 rounded-full blur-3xl mix-blend-multiply filter animate-blob animation-delay-4000"></div>
           </div>

          <div className="container mx-auto px-4 relative z-10">
            <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-24">
              <div className="flex-1 text-center lg:text-left space-y-8">
                <div className="inline-flex items-center rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1 text-sm text-cyan-800 font-medium mb-4">
                ...
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight text-gray-900 leading-[1.1]">
                  ...
                  <span className="text-cyan-500">Sem Fronteiras.</span>
                </h1>
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                  <Link href="/auth/register">
                    <Button size="lg" className="w-full sm:w-auto text-lg h-12 px-8 bg-cyan-600 hover:bg-cyan-700 shadow-lg hover:shadow-xl transition-all">
                ...
                <div className="pt-8 flex items-center justify-center lg:justify-start gap-6 text-sm text-gray-500 font-medium">
                  <span className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-cyan-500" /> Sem mensalidade</span>
                  <span className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-cyan-500" /> Cartão Virtual Grátis</span>
                  <span className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-cyan-500" /> Suporte 24/7</span>
                </div>
              </div>
              <div className="flex-1 relative w-full max-w-[500px] lg:max-w-none">
                <div className="relative rounded-2xl bg-white/80 backdrop-blur-sm p-4 shadow-2xl border border-white/50 rotate-1 hover:rotate-0 transition-transform duration-500">
                   <div className="absolute -top-12 -right-12 w-24 h-24 bg-cyan-100 rounded-full blur-2xl opacity-60"></div>
                   <div className="absolute -bottom-12 -left-12 w-32 h-32 bg-blue-100 rounded-full blur-2xl opacity-60"></div>
                   {/* Placeholder for Dashboard Image - In real app use a screenshot */}
                   <div className="rounded-xl w-full h-auto aspect-[4/3] bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center border border-gray-200">
                      <div className="text-center p-8">
                        <Logo className="w-24 h-24 mx-auto mb-4 opacity-80" />
                        <p className="text-gray-400 font-medium">Dashboard GlobalSecure</p>
                      </div>
                   </div>
        ...
        <section className="py-24 bg-cyan-600 relative overflow-hidden">
          <div className="absolute inset-0 bg-[url('/globe.svg')] opacity-10 bg-center bg-no-repeat bg-cover"></div>
          <div className="container mx-auto px-4 text-center relative z-10">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">
              Pronto para revolucionar suas finanças?
            </h2>
            <p className="text-cyan-100 text-lg max-w-2xl mx-auto mb-10">
              Junte-se a milhares de usuários que já confiam na GlobalSecure para suas transações internacionais.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/auth/register">
                <Button size="lg" className="bg-white text-cyan-600 hover:bg-cyan-50 hover:text-cyan-700 font-bold px-8 h-14 text-lg w-full sm:w-auto shadow-xl">
