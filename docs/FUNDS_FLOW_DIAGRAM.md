...
- **Plataforma GlobalSecureSend (GSS)**: backend Next.js + Prisma + Postgres; mantém saldo e trilha (`Balance`, `AccountTransaction`, `Transfer`). Ver [schema.prisma](../prisma/schema.prisma#L192-L275).
- **Stripe (Payments)**: entrada via cartão/checkout e webhooks. Ver [stripe webhook](../src/app/api/webhooks/stripe/route.ts).
- **Polygon (USDT)**: eventos de depósito via webhook/handler dedicado. Ver [usdt webhook](../src/app/api/webhooks/crypto/usdt/route.ts).
- **Emissor de cartão (Issuing)**: integração atual modelada como Stripe Issuing (com fallback/mock). Ver [stripe.ts](../src/lib/services/stripe.ts) e [card.ts](../src/lib/services/card.ts).
- **SEPA / PIX (depósito local)**: endpoints existentes para crédito local no ledger interno. Ver [sepa deposit](../src/app/api/wallet/deposit/sepa/route.ts) e [pix deposit](../src/app/api/wallet/deposit/pix/route.ts).
...
- **Evidência técnica**:
  - Handler de webhook: [webhooks/stripe/route.ts](../src/app/api/webhooks/stripe/route.ts)
  - Lançamentos no ledger interno: modelos `Balance` e `AccountTransaction` em [schema.prisma](../prisma/schema.prisma#L212-L241)
- **Saída esperada**:
...
- **Evidência técnica**:
  - Handler: [webhooks/crypto/usdt/route.ts](../src/app/api/webhooks/crypto/usdt/route.ts)
- **Saída esperada**:
...
- **Evidência técnica (modelo atual)**:
  - Funções de emissão/controle: [stripe.ts](../src/lib/services/stripe.ts)
  - Consulta de dados do cartão (com fallback/mock): [card.ts](../src/lib/services/card.ts)
...
- **Evidência técnica**:
  - Endpoint e regra UE: [wallet/deposit/sepa/route.ts](../src/app/api/wallet/deposit/sepa/route.ts)
- **Saída esperada**:
...
- **Evidência técnica**:
  - Endpoint: [wallet/deposit/pix/route.ts](../src/app/api/wallet/deposit/pix/route.ts)
- **Saída esperada**:
...
- **Evidência operacional**: usar `jest-results.json` + [SECURITY_TEST_REPORT.md](./SECURITY_TEST_REPORT.md) como material de reunião, anexando logs/prints quando necessário.
